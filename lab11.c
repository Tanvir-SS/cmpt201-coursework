...
int main() {
  ...
  if (!pubkey_file) {
    handle_error("Unable to open public key file");
  }
  ...
  if (!pubkey) {
    handle_error("Unable to read public key");
  }
...
#define MAX_FILE_SIZE 512
  ...
  status = EVP_DigestVerifyFinal(ctx, signature, signature_len);
  // status: 1 = success, 0 = invalid, <0 = error
